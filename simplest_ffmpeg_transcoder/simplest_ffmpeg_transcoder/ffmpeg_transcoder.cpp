...
static int open_input_file(const char *filename)
{
	...
	for(i = 0; i < ifmt_ctx->nb_streams; ++i)
	{
		...
		if(AVMEDIA_TYPE_VIDEO == codec_ctx->codec_type|| AVMEDIA_TYPE_AUDIO == codec_ctx->codec_type)
		{
			//Open decoder
...
static int open_out_file(const char *filename)
{
	...
	for(i = 0; i < ifmt_ctx->nb_streams; ++i)
	{
		stream_out = avformat_new_stream(ofmt_ctx, NULL);
		if(!stream_out)
		{
			av_log(NULL, AV_LOG_ERROR, "Failedallocating output stream\n");
			return AVERROR_UNKNOWN;
		}
		stream_in = ifmt_ctx->streams[i];
		dec_ctx = stream_in->codec;
		enc_ctx = stream_out->codec;
		if (AVMEDIA_TYPE_VIDEO == dec_ctx->codec_type || AVMEDIA_TYPE_AUDIO == dec_ctx->codec_type) 
		{
		...
		else if(AVMEDIA_TYPE_UNKNOWN == dec_ctx->codec_type)
		{
...
static int encode_write_frame(AVFrame *filt_frame, unsigned int stream_index, int *got_frame)
{
	...
	int (*enc_func)(AVCodecContext *, AVPacket *, const AVFrame *, int*) =
		( AVMEDIA_TYPE_VIDEO == ifmt_ctx->streams[stream_index]->codec->codec_type) ? avcodec_encode_video2 : avcodec_encode_audio2;
...
int main(int argc, char **argv)
{
	...
	unsigned int i            = 0;
	int      got_frame        = 0;
	const char *filename_in   = "../cuc_ieschool.ts";
	const char *filename_out  = "1.mp4";
	...
	if ((ret = open_out_file(filename_out)) < 0)
		goto end;
	...
	while (1) 
	{
		...
		if (filter_ctx[stream_index].filter_graph) 
		{
			...
			packet.pts = av_rescale_q_rnd(packet.pts,
				...
				ifmt_ctx->streams[stream_index]->codec->time_base,
				(AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
			dec_func = (AVMEDIA_TYPE_VIDEO == type) ? avcodec_decode_video2 :
